...
class Wechat extends Base {
    ...
    public function pub_menu(){
        ...
        $access_token = $this->get_access_tokenNew($wechat['appid'],$wechat['appsecret']);
        if(!$access_token){
            $this->error('获取access_token失败-'.$access_token,U('Wechat/menu')); 
            exit;
        }
    ...
    /**
     * 获取token
     * @author lishibo 20190713
     * @param $appid
     * @param $appsecret
     * @return mixed
     */
    public function get_access_tokenNew($appid,$appsecret){
        //判断是否过了缓存期
        $wechat = M('wx_user')->find();
        ...
        $return = json_decode($this->str5($return),1);

        if(!$return['access_token']){
            echo "错误代码;".$return['errmsg'];
