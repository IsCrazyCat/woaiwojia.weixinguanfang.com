   ...
   return $freight;  
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        $data = array(
                ...
                'user_money'       =>$car_price['balance'],//'使用金币',
                'coupon_price'     =>$car_price['couponFee'],//'使用优惠券',                        
                'integral'         =>($car_price['pointsFee'] * tpCache('shopping.point_rate')), //'使用银币',
                'integral_money'   =>$car_price['pointsFee'],//'使用银币抵多少钱',
        ...
        if($car_price['pointsFee']>0)
        	M('Users')->where("user_id = $user_id")->setDec('pay_points',($car_price['pointsFee'] * tpCache('shopping.point_rate'))); // 用户的银币减 
        if($car_price['balance']>0)
        	M('Users')->where("user_id = $user_id")->setDec('user_money',$car_price['balance']); // 用户的金币减         
        // 4 删除已提交订单商品
